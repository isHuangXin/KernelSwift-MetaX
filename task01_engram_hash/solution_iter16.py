...
def _engram_kernel(
    ...
    BLOCK_M: tl.constexpr, BLOCK_C: tl.constexpr, NG_P: tl.constexpr,
):
    ...
    # load all NG token cols at once: (BLOCK_M, NG_P)
    s_idx = tl.arange(0, NG_P)
    s_mask = s_idx < NG
    tok_tile = tl.load(tok_ptr + m_idx[:, None] * NG + s_idx[None, :],
                       mask=m_mask[:, None] & s_mask[None, :], other=0).to(tl.int64)  # (BLOCK_M, NG_P)
    mult_tile = tl.load(mult_ptr + layer * NG + s_idx, mask=s_mask, other=0)          # (NG_P,)
    prod = tok_tile * mult_tile[None, :]                                              # (BLOCK_M, NG_P)
    run = tl.zeros((BLOCK_M,), dtype=tl.int64)
    hash_mc = tl.zeros((BLOCK_M, BLOCK_C), dtype=tl.int64)
    for s in range(0, NG):
        col = tl.sum(tl.where(s_idx[None, :] == s, prod, 0), axis=1)   # (BLOCK_M,) = prod[:,s]
        run = run ^ col
...
def _engram_triton(tok, mult, vocab_flat, off_flat, L, num_tokens, NG, T, BLOCK_M=16):
    COLS = (NG - 1) * T
    ...
    _engram_kernel[grid](
        ...
        num_tokens, L, NG, T, COLS,
        BLOCK_M=BLOCK_M, BLOCK_C=BLOCK_C, NG_P=triton.next_power_of_2(NG),
    )
    return out
