...
def _engram_kernel(
    ...
    m_idx = m0 + tl.max_contiguous(tl.arange(0, BLOCK_M), BLOCK_M)          # token rows
    c_idx = tl.max_contiguous(tl.arange(0, BLOCK_C), BLOCK_C)               # columns
...
def _engram_triton(tok, mult, vocab_flat, off_flat, L, num_tokens, NG, T, BLOCK_M=32):
    COLS = (NG - 1) * T
