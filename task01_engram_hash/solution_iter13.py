...
@triton.jit
def _engram_kernel(
    tok_ptr, mult_ptr, vocab_ptr, magic_ptr, off_ptr, out_ptr,
    num_tokens, L, NG, T, COLS, S,
    ...
    for s in range(0, NG):
        tk = tl.load(tok_ptr + m_idx * NG + s, mask=m_mask, other=0).to(tl.int64)
        mu = tl.load(mult_ptr + layer * NG + s)
        run = run ^ (tk * mu)
        hash_mc = tl.where((i + 1)[None, :] == s, run[:, None], hash_mc)

    vs = tl.load(vocab_ptr + layer * COLS + c_idx, mask=c_mask, other=1).to(tl.int64)
    magic = tl.load(magic_ptr + layer * COLS + c_idx, mask=c_mask, other=1).to(tl.int64)
    off = tl.load(off_ptr + layer * COLS + c_idx, mask=c_mask, other=0)
    # magic-number modulo: q = (hash * magic) >> S ; r = hash - q*vocab ; 1-step correct
    q = (hash_mc * magic[None, :]) >> S
    r = hash_mc - q * vs[None, :]
    r = tl.where(r >= vs[None, :], r - vs[None, :], r)
    r = tl.where(r < 0, r + vs[None, :], r)
...
def _engram_triton(tok, mult, vocab_flat, magic_flat, off_flat, L, num_tokens, NG, T, BLOCK_M=16):
    COLS = (NG - 1) * T
    ...
    _engram_kernel[grid](
        tok, mult, vocab_flat, magic_flat, off_flat, out,
        num_tokens, L, NG, T, COLS, _MAGIC_S,
...
class ModelNew(nn.Module):
    ...
    def forward(self, ngram_token_ids, multipliers, vocab_sizes, offsets):
        ...
        # precompute per-vocab magic = floor(2^S / vocab) + 1  (int64)
        magic_flat = ((1 << _MAGIC_S) // vocab_flat.to(torch.int64) + 1).contiguous()
        return _engram_triton(tok, mult, vocab_flat, magic_flat, off_flat, L, num_tokens, NG, T)
