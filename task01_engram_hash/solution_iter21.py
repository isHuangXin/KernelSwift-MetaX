...
@triton.jit
def _engram_kernel(
    tok_ptr, mult_ptr, vo_ptr, out_ptr,
    ...
    base = (layer * COLS + c_idx) * 2
    vs = tl.load(vo_ptr + base, mask=c_mask, other=1).to(tl.int64)
    off = tl.load(vo_ptr + base + 1, mask=c_mask, other=0)
    r = hash_mc % vs[None, :]
...
def _engram_triton(tok, mult, vo, L, num_tokens, NG, T, BLOCK_M=32):
    COLS = (NG - 1) * T
...
class ModelNew(nn.Module):
    ...
    def forward(self, ngram_token_ids, multipliers, vocab_sizes, offsets):
        ...
        vo = torch.stack([vocab_flat, off_flat], dim=-1).contiguous()  # (L, COLS, 2)
        return _engram_triton(tok, mult, vo, L, num_tokens, NG, T)
