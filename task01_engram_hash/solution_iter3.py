...
def _engram_kernel(
    ...
    # scalar running xor; column picks hash after step (i+1). NG steps.
    run = tl.zeros((), dtype=tl.int64)
    hash_c = tl.zeros((BLOCK_C,), dtype=tl.int64)
    for s in range(0, NG):
        tk = tl.load(tok_ptr + tok * NG + s).to(tl.int64)
        mu = tl.load(mult_ptr + layer * NG + s)
        run = run ^ (tk * mu)
        # after including step s, columns with i+1 == s take this running hash
        hash_c = tl.where((i + 1) == s, run, hash_c)
