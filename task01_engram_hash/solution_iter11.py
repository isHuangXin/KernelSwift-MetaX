...
def _engram_kernel(
    ...
    c_mask = c_idx < COLS
    i = c_idx // T                              # ngram step per column (0..NG-2)

    # compute running xor; keep only the two hash snapshots (after step1, step2)
    run = tl.zeros((BLOCK_M,), dtype=tl.int64)
    h0 = tl.zeros((BLOCK_M,), dtype=tl.int64)   # hash after including step 1 (i==0)
    h1 = tl.zeros((BLOCK_M,), dtype=tl.int64)   # hash after including step 2 (i==1)
    for s in range(0, NG):
        tk = tl.load(tok_ptr + m_idx * NG + s, mask=m_mask, other=0).to(tl.int64)
        mu = tl.load(mult_ptr + layer * NG + s)
        run = run ^ (tk * mu)
        h0 = tl.where(s == 1, run, h0)
        h1 = tl.where(s == 2, run, h1)
    # gather hash per column: i==0 -> h0, i==1 -> h1
    hash_mc = tl.where((i == 0)[None, :], h0[:, None], h1[:, None])
