...
# sweep SPLIT_K / BLOCK_K / num_warps at the python level, cache best per (M,N,K)
_BEST = {}
_CANDIDATES = [
    (8, 512, 4), (16, 512, 4), (16, 256, 4), (32, 256, 4),
    (32, 128, 4), (16, 512, 8), (32, 512, 8), (64, 128, 4), (64, 256, 4),
]


def _run(residual_2d, fn2d, eps, SPLIT_K, BLOCK_K, WARPS):
...
def _norm_fn_triton(residual_2d, fn2d, eps):
    ...
    if key not in _BEST:
        import time
        best_cfg, best_t = None, 1e9
        for cfg in _CANDIDATES:
            try:
                for _ in range(3):
                    _run(residual_2d, fn2d, eps, *cfg)
                ...
                if dt < best_t:
                    best_t, best_cfg = dt, cfg
            except Exception:
                continue
        _BEST[key] = best_cfg or _CANDIDATES[1]
    return _run(residual_2d, fn2d, eps, *_BEST[key])
