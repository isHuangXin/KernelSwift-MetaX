...
def _partial_kernel(
    ...
    tl.store(part_ptr + pid_m * N * SPLIT_K + n_idx * SPLIT_K + pid_k, acc, mask=n_mask)
...
def _reduce_kernel(
    ...
    p = tl.load(
        part_ptr + pid_m * N * SPLIT_K + n_idx[:, None] * SPLIT_K + s_idx[None, :],
        mask=n_mask[:, None] & s_mask[None, :], other=0.0,
    )
    acc = tl.sum(p, axis=1)
