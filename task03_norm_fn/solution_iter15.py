...
def _partial_kernel(
    ...
    # grid = (SPLIT_K,) — each program handles one K-chunk for ALL M rows.
    # fn tile is loaded once per K-chunk and reused across all M rows (major HBM saving).
    ...
    for k0 in range(k_start, k_stop, BLOCK_K):
        ...
        # fn_tile: (BLOCK_N, BLOCK_K) — loaded once, reused for all M
        fn_tile = tl.load(
            fn_ptr + n_idx[:, None] * K + k_idx[None, :],
            mask=n_mask[:, None] & k_mask[None, :], other=0.0,
        ).to(tl.float32)
        # acc[m,n] += sum_k r[m,k]*fn[n,k]  -> use dot
        acc += tl.dot(r, tl.trans(fn_tile))
    # part: (SPLIT_K, M, N), ssum: (SPLIT_K, M)
    tl.store(
        part_ptr + pid_k * M * N + m_idx[:, None] * N + n_idx[None, :],
...
def _reduce_kernel(
    ...
    # ssum: (SPLIT_K, M) -> gather column pid_m
    ss = tl.load(ssum_ptr + s_idx * M + pid_m, mask=s_mask, other=0.0)
    rms = tl.math.rsqrt(tl.sum(ss) / K + eps)
    # part: (SPLIT_K, M, N) -> gather [:, pid_m, :]
    p = tl.load(
        part_ptr + s_idx[:, None] * M * N + pid_m * N + n_idx[None, :],
