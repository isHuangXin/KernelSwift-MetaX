...
def _reduce_kernel(
    ...
    M, N, K, SPLIT_K, eps,
    BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, BLOCK_S: tl.constexpr,
):
    # single program handles ALL (M,N): loop over splits, accumulate.
    ...
    # ssum: (M, SPLIT_K) -> (BLOCK_M, BLOCK_S)
    ss = tl.load(ssum_ptr + m_idx[:, None] * SPLIT_K + s_idx[None, :],
                 mask=m_mask[:, None] & s_mask[None, :], other=0.0)
    sqsum = tl.sum(ss, axis=1)                      # (BLOCK_M,)
    rms = tl.math.rsqrt(sqsum / K + eps)            # (BLOCK_M,)
    # part: (M, SPLIT_K, N); accumulate over splits
    acc = tl.zeros((BLOCK_M, BLOCK_N), dtype=tl.float32)
    for s in range(0, SPLIT_K):
        p = tl.load(part_ptr + m_idx[:, None] * SPLIT_K * N + s * N + n_idx[None, :],
                    mask=m_mask[:, None] & n_mask[None, :], other=0.0)
        acc += p
    out = acc * rms[:, None]
    tl.store(out_ptr + m_idx[:, None] * N + n_idx[None, :],
...
def _norm_fn_triton(residual_2d, fn2d, eps):
    ...
    _reduce_kernel[(1,)](
        ...
        BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N, BLOCK_S=BLOCK_S, num_warps=2,
    )
    return out
