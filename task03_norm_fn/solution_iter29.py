...
def _partial_kernel(
    ...
    tl.store(part_ptr + (pid_m * SPLIT_K + pid_k) * N + n_idx, acc.to(tl.bfloat16), mask=n_mask)
...
def _norm_fn_triton(residual_2d, fn2d, eps):
    ...
    part = torch.empty((M, SPLIT_K, N), dtype=torch.bfloat16, device=residual_2d.device)
